...
class UpdateServiceTest extends Specification {
    ...
    def "update"() {
        ...
        when:
        def result = new UpdateService(pfxClient, PFXTypeCode.ROLE, "USER").execute(null, request.toString())

        then:
        Boolean.TRUE.toString() == result.get(PFXConstants.FIELD_VALUE).textValue()

        when:
        new UpdateService(pfxClient, PFXTypeCode.PRODUCT, "TEST").execute(null, request.toString())
