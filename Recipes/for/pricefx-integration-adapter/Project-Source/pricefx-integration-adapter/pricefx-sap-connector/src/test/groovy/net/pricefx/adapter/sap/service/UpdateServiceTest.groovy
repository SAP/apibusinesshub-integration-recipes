...
class UpdateServiceTest extends Specification {
    ...
    def "update"() {
        ...
        when:
        def result = new UpdateService(pfxClient, PFXTypeCode.ROLE,  null, "USER",null).execute(request.toString())

        then:
        Boolean.TRUE.toString() == result.get(PFXConstants.FIELD_VALUE).textValue()

        when:
        new UpdateService(pfxClient, PFXTypeCode.PRODUCT,  null, "TEST", null).execute(request.toString())
