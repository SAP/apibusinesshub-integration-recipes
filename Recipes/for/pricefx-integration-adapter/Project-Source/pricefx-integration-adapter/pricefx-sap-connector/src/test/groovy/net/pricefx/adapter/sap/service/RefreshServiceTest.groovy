...
class RefreshServiceTest extends Specification {
    ...
    def "refresh"() {
        when:
        def result = new RefreshService(pfxClient, PFXTypeCode.TOKEN, "dummy", null).refresh(null)

        then:
        "dummy-access-token" == result.get("access-token").textValue()

        when:
        new RefreshService(pfxClient, PFXTypeCode.TOKEN, null, null).refresh(null)
        ...
        when:
        new RefreshService(pfxClient, PFXTypeCode.PRODUCT, null, null).refresh(null)
