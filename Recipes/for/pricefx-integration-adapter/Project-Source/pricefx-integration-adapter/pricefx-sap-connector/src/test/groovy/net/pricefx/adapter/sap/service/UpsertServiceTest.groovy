...
class UpsertServiceTest extends Specification {
    ...
    def "upsert"() {
        ...
        when:
        def result = new UpsertService(pfxClient, PFXTypeCode.PRODUCT, null, false, false).execute(null, request.toString())

        then:
        original.size() == result.size()

        when:
        result = new UpsertService(pfxClient, PFXTypeCode.PRODUCT, null, true, false).execute(null, request.toString())
        ...
        result = new UpsertService(pfxClient, PFXTypeCode.PRODUCT, null, false, false).execute(null, request.toString())

        then:
        result.isObject()

        when:
        result = new UpsertService(dummyClient, PFXTypeCode.PRODUCT, null, false, false).execute(null, request.toString())
