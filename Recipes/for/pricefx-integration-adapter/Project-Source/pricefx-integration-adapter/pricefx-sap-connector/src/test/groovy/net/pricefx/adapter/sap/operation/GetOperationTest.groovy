...
class GetOperationTest extends Specification {
    ...
    def "get"() {
        when:
        def result = new GetOperation(pfxClient, PFXTypeCode.PRODUCT, "TEST", null, (IPFXExtensionType) null).get(null, 0, Constants.MAX_RECORDS)

        then:
        "TEST" == result.get(PFXConstants.FIELD_SKU).textValue()

        when:
        result = new GetOperation(pfxClient, PFXTypeCode.QUOTE, "TEST", null, (IPFXExtensionType) null).get(null, 0, Constants.MAX_RECORDS)
        ...
        when:
        new GetOperation(pfxClient, PFXTypeCode.PRODUCTIMAGE, "TEST", null, (IPFXExtensionType) null).get(null, 0, Constants.MAX_RECORDS)
        ...
        new GetOperation(pfxClient, PFXTypeCode.LOOKUPTABLE, "TEST", null, matrix).get(null, 0, Constants.MAX_RECORDS)
        ...
        result = new GetOperation(pfxClient, PFXTypeCode.LOOKUPTABLE, "TEST", null, matrix).get(null, 0, Constants.MAX_RECORDS)
